...
# ============================================================
# DSPR parameter extraction from the cleaned Excel protocol
# ============================================================
...
def classify_segment(voltage: float, note: str, v_read_nominal: float = 1.0) -> str:
    ...
    reset_keywords = [
        "reset", "reseteaza", "reseta", "reseta m", "puls de reset", "ca sa resetam"
    ]
...
def build_transform(dataset_name: str, train: bool):
    ...
    if "emnist" in name_l:
        base.append(transforms.Lambda(emnist_fix))
...
def get_dataset(name: str, data_dir: str, train: bool):
    ...
    try:
        ...
        if name_l == "kmnist":
            ds = datasets.KMNIST(root=data_dir, train=train, download=True, transform=tfm)
            return ds, 10
        ...
        if name_l == "emnist-balanced":
            ds = datasets.EMNIST(root=data_dir, split="balanced", train=train, download=True, transform=tfm)
            return ds, 47
        ...
        if name_l == "qmnist":
            ...
            ds = datasets.QMNIST(root=data_dir, what=what, download=True, transform=tfm, compat=True)
            return ds, 10
    ...
    raise ValueError(
        f"Unsupported dataset: {name}. Use FashionMNIST, EMNIST-Balanced, "
        f"EMNIST-Letters, QMNIST, KMNIST, or MNIST."
    )
...
class LearnableLIFCell(nn.Module):
    def __init__(self, beta_init=0.90, threshold=1.0, reset="subtract", spike_slope=10.0):
        super().__init__()
...
class LearnableLICell(nn.Module):
    def __init__(self, beta_init=0.90):
        super().__init__()
        beta_init = float(np.clip(beta_init, 1e-4, 1.0 - 1e-4))
        rho = math.log(beta_init / (1.0 - beta_init))
        self.rho = nn.Parameter(torch.tensor(rho, dtype=torch.float32))
...
class DSPRBase(nn.Module):
    def __init__(
        ...
        zpos = torch.empty(*param_shape).normal_(mean=mean, std=init_spread).clamp_(0.0, 1.0)
        zneg = torch.empty(*param_shape).normal_(mean=mean, std=init_spread).clamp_(0.0, 1.0)
...
class ConvDSPRNet(nn.Module):
    """
    Convolutional SNN with DSPR-constrained synapses throughout.
    """
    def __init__(
        self,
        n_out: int,
        ...
        beta_out: float = 0.90,
        syn_scale_conv1: float = 1.0,
        syn_scale_conv2: float = 1.0,
        syn_scale_fc1: float = 1.0,
        syn_scale_fc2: float = 1.0,
    ...
    def zero_device_grads(self) -> None:
...
def output_logits(mem2_rec: torch.Tensor, mode: str) -> torch.Tensor:
    if mode == "sum":
        return mem2_rec.sum(dim=0)
    if mode == "max":
        return mem2_rec.max(dim=0).values
    if mode == "last":
        return mem2_rec[-1]
    raise ValueError("mode must be one of sum|max|last")
...
def make_loader(ds, batch_size: int, train: bool):
    """
    Conservative DataLoader settings for macOS / MPS debugging.
    """
    return DataLoader(
        ds,
...
def train_one_epoch(model, loader, device, args):
    model.train()
    ...
    for xb, yb in loader:
        ...
        if args.fr_lambda > 0.0:
            if not torch.isfinite(mean_fr):
                raise RuntimeError("Non-finite hidden firing-rate term detected in training.")
        ...
        model.dspr_step(args.eta_net)

        batch_n = xb.size(0)
        batch_correct = int((logits.argmax(dim=1) == yb).sum().item())
        batch_fr = float(mean_fr.item())

        total_loss += float(loss.item()) * batch_n
        total_correct += batch_correct
        total_n += batch_n
        mean_hidden_fr += batch_fr * batch_n
    ...
    if not np.isfinite(avg_loss):
        raise RuntimeError(f"Non-finite train loss detected at epoch end: {avg_loss}")
    if not np.isfinite(acc):
        raise RuntimeError(f"Non-finite train accuracy detected at epoch end: {acc}")
    if acc < 0.0 or acc > 1.0:
        raise RuntimeError(f"Impossible train accuracy detected: {acc}")
    if not np.isfinite(fr_hid):
        raise RuntimeError(f"Non-finite hidden firing rate detected at epoch end: {fr_hid}")
...
def eval_one_epoch(model, loader, device, args):
    ...
    if not np.isfinite(avg_loss):
        raise RuntimeError(f"Non-finite eval loss detected at epoch end: {avg_loss}")
    if not np.isfinite(acc):
        raise RuntimeError(f"Non-finite eval accuracy detected at epoch end: {acc}")
    if acc < 0.0 or acc > 1.0:
        raise RuntimeError(
            f"Impossible eval accuracy detected: {acc}. "
            f"This usually points to a backend/runtime issue."
        )
...
def train_dataset(dataset_name: str, dspr: DSPRParams, args):
    ...
    for epoch in range(1, args.epochs + 1):
        tr = train_one_epoch(model, train_loader, device, args)
...
# ============================================================
# Optional topology comparison from the positive-only raw TSVs
# ============================================================
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Improved DSPR-constrained convolutional SNN on harder MNIST-family datasets"
    )
    ap.add_argument("--protocol_xlsx", type=str, required=True, help="Cleaned read/set/reset protocol Excel file")
    ap.add_argument("--sheet_name", default="0", help="Excel sheet index or name (default: 0)")
    ap.add_argument(
        "--datasets",
        type=str,
        default="FashionMNIST,EMNIST-Balanced,EMNIST-Letters",
        help="Comma-separated list from: FashionMNIST, EMNIST-Balanced, EMNIST-Letters, QMNIST, KMNIST, MNIST",
    )
    ap.add_argument("--data_dir", type=str, default="./data")
    ap.add_argument("--epochs", type=int, default=25)
    ap.add_argument("--batch_size", type=int, default=128)
    ap.add_argument("--train_subset", type=int, default=0, help="0 = full train split")
    ap.add_argument("--test_subset", type=int, default=0, help="0 = full test split")
    ...
    ap.add_argument("--rate_scale", type=float, default=0.35)
    ap.add_argument("--latency_thr", type=float, default=0.15)
    ap.add_argument("--mem_logits", type=str, default="sum", choices=["sum", "max", "last"])
    ap.add_argument("--temporal_loss_weight", type=float, default=0.35,
                    help="Blend between readout CE and temporal CE across all time steps")

    # Neuron parameters
    ap.add_argument("--beta_h", type=float, default=0.92)
    ap.add_argument("--thr_h", type=float, default=1.0)
    ...
    ap.add_argument("--beta_out", type=float, default=0.95)

    # Synaptic current scaling
    ap.add_argument("--syn_scale_conv1", type=float, default=1.1)
    ap.add_argument("--syn_scale_conv2", type=float, default=1.0)
    ap.add_argument("--syn_scale_fc1", type=float, default=0.9)
    ap.add_argument("--syn_scale_fc2", type=float, default=0.9)

    # DSPR / physical update
    ap.add_argument("--eta_net", type=float, default=5e-3, help="Learning signal scale before DSPR projection")
    ...
    ap.add_argument("--init_spread", type=float, default=0.02)
    ap.add_argument("--delta_z_cap", type=float, default=0.05,
                    help="Max desired normalized state increase per optimizer step")
    ap.add_argument("--noise_on", action="store_true")
    ap.add_argument("--leak_dt", type=float, default=0.0,
                    help="Passive 0V relaxation time applied once per optimizer step")

    # Optional history term
    ...
    # Optional auxiliary raw files for topology comparison
...
def main():
    ...
    topo_paths = {}
    if args.raw_1m:
        topo_paths["1m"] = args.raw_1m
    if args.raw_1p1m:
        topo_paths["1+1m"] = args.raw_1p1m
    if args.raw_2p1m:
        topo_paths["2+1m"] = args.raw_2p1m
    if args.raw_3m:
        topo_paths["3m"] = args.raw_3m
